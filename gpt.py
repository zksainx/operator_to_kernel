...
class GPT(nn.Module):
    def __init__(self,d_model,nhead,feedforward,vocab_size,seq_max_len):
        ...
        self.dec_blocks=nn.ModuleList([
            nn.TransformerEncoderLayer(d_model=d_model,nhead=nhead,dim_feedforward=feedforward,batch_first=True) for _ in range(6)
        ])
...
if __name__=='__main__':
    ...
    # 模拟输入
    x=torch.randint(0,5000,(5,30))
    padding=torch.zeros(5,30)
    
    # GPT模型
    gpt=GPT(d_model=64,nhead=2,feedforward=128,vocab_size=5000,seq_max_len=50)
    y=gpt(x,padding)
    print(y.shape)